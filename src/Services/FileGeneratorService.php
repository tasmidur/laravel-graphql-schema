...
class FileGeneratorService
{
    ...
    {
        [$fields, $args, $validationRules, $argsWithValidation] = $rules;
        ...
        $parsedData = $this->parseRulesAndArgs($fields, $args, $validationRules, $argsWithValidation);
    ...
    private function parseRulesAndArgs(array $fields, array $args, array $validationRules, $argsWithValidation): array
    {
        return [
            'fields' => GraphQLHelper::formatForFile($fields),
            'args' => GraphQLHelper::formatForFile($args),
            'createArgs' => GraphQLHelper::formatForFile(array_values(array_filter($argsWithValidation, fn($value,$key) => $value['name'] !== "id", ARRAY_FILTER_USE_BOTH))),
            'updateArgs' => GraphQLHelper::formatForFile($argsWithValidation),
            'validationRulesForCreate' => GraphQLHelper::formatForFile(
                array_filter($validationRules, fn($key) => $key !== "id", ARRAY_FILTER_USE_KEY),
                GraphQLHelper::$patternValidation,
                GraphQLHelper::$replacerValidation,
            ),
            'validationRulesForUpdate' => GraphQLHelper::formatForFile(
                $validationRules,
                GraphQLHelper::$patternValidation,
                GraphQLHelper::$replacerValidation,
            )
